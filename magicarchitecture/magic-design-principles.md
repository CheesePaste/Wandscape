...
### P13. 转速看"重叠/相切"，不看半径 —— 重叠必须同速，相切建议异向
两个图形的角速度该不该相同，取决于它们的**位置关系**，而不是半径或"层级"：

- **重叠**（粒子占据同一区域/交错）：必须同角速度，否则互相打架、糊。
  - 与某形状特征必须对齐的元素（如符文对准星形内凹顶点）即便不重叠，也是**同一刚体**，必须同速——例：符文 r2.75 与星星 r5.5 半径差大，但同 +8°/s。
  - 同层元素通常交错重叠，故共享 `rotate_speed`（旧"同层同速"是其特例）。
- **相切**（只接触、不重叠）：可以且**建议不同角速度**（异向），制造视差。
- 判断法：算两元素的粒子落点是否共点/共区。设计时先保证几何关系（相切或分离），再据此定转速。
- 反例（会翻车）：符文 r4.6 与星星 r5.5 同心内叠、各转各的 → 两个图案打架、糊。
