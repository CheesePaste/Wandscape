...
### P13. 同层级同角速度 — 先分层，再定转速
旋转是"层级"的属性，不是"元素"的属性。同一视觉层级的元素共享 `rotate_speed`，层内用 `rotation_offset_deg` 对齐特征。

- **同一视觉层级** = 半径差 < ~15% 且同高度同朝向的元素（如同轴地面阵）。
- 同层所有元素**共享 `rotate_speed`**；层内用 `rotation_offset_deg` 对齐特征（如符文对准内凹顶点）。
- **刚体例外**：与某形状特征必须保持对齐的元素（如符文对准星形内凹顶点）即使半径不同，也必须与该形状同角速度——它们是同一个刚体。例：符文 r2.75 与星星 r5.5 半径差大，但同 +8°/s。
- **相邻层级异向**（P1），层级感来自半径差 + 转速差。
- 反例（会翻车）：符文 +16°/s 与星星 +8°/s 半径相近、同高度，各转各的 → 两个同心图案打架、糊。

### P14. 符文/子元素放"凹陷处"不放顶点（含防重叠）
多边形/星形的**外顶点是"锋芒"**，把符文也放顶点会簇拥、其它区域留空（空一块密一块）。符文应放**凹陷处**，视觉密度才均匀。

- **星形**：符文放**内凹顶点**——半径 = 外半径 × `inner_ratio`，角度对齐内凹顶点角（六芒星 R=5.5、inner 0.5 → 符文 r=2.75、角度 30°+60k）。
- **多边形**：符文放角间/边中点（半径 × cos(π/sides)，角度错半周期）。
- **防重叠（关键）**：星形内凹顶点与"放那里的符文"坐标**完全重合**（同半径同角度）。所以星形自身的 beads **只放外顶点**，内凹位留给符文——二者合起来才是完整形状，互不重叠、密度均匀。
- 设计自查：任何子元素与宿主形状的顶点/边坐标重合即视为**重叠**，须用"宿主让位"（外顶点）或"错位"（角间/内缩半径）规避。
