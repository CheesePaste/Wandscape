...
| polygon | sides | 顶点数（≥3） |
| polygon | mode / density / trail_ticks / y_offset / interval_ticks | 同 ring（beads 时沿周长均匀描边，`density` 控制描边密度） |
...
- **polygon/star**：`beads`（默认）= 沿周长按 `density×周长` 均匀撒点（跨角不断、密度一致）；`continuous` = 沿各边动态拖尾。星形首尖朝上。新建默认自转 + 持续扩大（`rotate_speed` + `anim.scale`）。
