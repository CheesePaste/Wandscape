...
文档编号：NEW-07
版本：1.1
状态：NPC 实体 + 属性 + 个人魔力 + 死亡/坟墓 + 房屋绑定（**完整设计目标**）—— 移动已改为直接传送
依赖：01-shared-api
...
NPC 移动通过 `WandscapeMovementOps.navigateTo()` 直接传送到目标位置（`npc.setPos()`），不走寻路。传送前检查水平距离：≤5 格视为已在范围内，跳过传送。

传送是确定性的——在动态建筑工地上寻路根本不可靠（方块不断被放置/移除，路径反复作废，NPC 被围困），直接传送对殖民地自动化模组完全可接受。

### 2.2 卡死判定

~~NPC 开始寻路步行后，每 `STUCK_CHECK_INTERVAL_TICKS`（3 秒）检查……~~

**已移除**：移动改为直接传送后，卡死检测不再需要。寻路及其所有相关机制（卡死检测、超时兜底、self_teleport 私有任务）均已在阶段 2 实现中移除。

### 2.3 外观

- 统一使用"法师村民"模型（复用 Player 模型）
- **多皮肤变体**：`textures/entity/wizard/` 文件夹下放置多套 png 皮肤，启动时自动检测数量。每个 NPC 首次生成时随机分配一个变体，持久化在 NBT
- **法师帽**：通过 `WizardHatLayer` 渲染在头顶，由三层递减 cuboid 堆叠（帽檐+锥体+尖端）。帽檐外圈金色，帽身颜色按变体轮转（7 色）
- 手持法杖显示在右手
- 空闲时收起法杖，工作时举起
- 无职业着装差异（法杖即能力，皮肤+帽子颜色区分个体）
...
NPC 生命值 → 0：
1. 持有的全局任务通过 `releaseTaskForReassign()` 释放回全局池（保留 stepIndex，私有池丢弃）
2. NPC 实体移除，ECS 组件注销
3. 背包内所有物品以掉落物形式散落在死亡位置
4. 触发 `NpcDiedEvent`（携带死亡位置和 NPC 数据）
5. 玩家可在掉落物消失前拾取装备
