...
| 钱包 | 万能元素（不分元素），等级越高初始资金越多（base + level×perLevel）。钱包为 0 后不再前往商店 |
...
- 每日清晨统一进货（与游客生成同一心跳）
- 进货从殖民地仓库直接扣物品（移动到商店库存），不扣元素
- 仓库缺货时自动合成补充：`ShopStockManager.restock` 入队 `production:synthesize`（若该物品有配方），合成缺元素再走 node 采集；物品入仓后每 ~100 tick 重试补齐店铺
- 补货到 `max_stock` 上限
...
- 游客携带一个**万能元素钱包**（不分元素），初始资金 = `tourist.base_wallet + 等级 × tourist.wallet_per_level`（Config）。
- 商品对钱包的定价 = 各元素利润之和（Σ 元素 × (1 + profit_rate)）。例：某货 10 水 + 10 火、利润率 30% → 卖 1 件殖民地得 13 水 + 13 火，钱包扣 26。
- **游客可超支购买**：买后钱包可为负、随即归 0。因此前期商店里游客买不起的高级货也会被消耗；钱包归 0 的游客不再前往商店（服务建筑仍可逛）。
- **每次购物经费**：随机取比例 a ∈ [0.2, 1]，经费 = min(a × 初始钱包, 当前余额)——避免首次购物一次性花光钱包。
- **购买流程**：先随机选一件有货商品，买 `floor(经费/单价) + 1` 件（+1 保证贵重品至少买 1 件），封顶库存，再从钱包扣实际金额。
- 效果：便宜货走量（面包一次买很多），贵重货走价（一次一件就掏空钱包）。

### 货品与三值联动

- 有货 → 货物对三值加成正常计入
- 缺货（仓库物品不足，且无合成配方/采集渠道可自动补充）→ 该建筑三值加成清零
