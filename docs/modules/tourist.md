...
- **满意度→经验**：离境且 sat=100 → ColonyLevelManager.computeExpContribution：游客等级=殖民地→`colony.expEqualLevel`（默认 700）、>→`colony.expAboveLevel`（默认 1400）、<→一半。升级公式 `expToNext=(level+1)×500`（1→2=1000、每级 +500），等级上限 `colony.maxLevel`（默认 31）。创始人离线时经验 × `colony.offlineIncomeMultiplier`。
