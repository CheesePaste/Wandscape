...
- `MagicCircleEmitter`：静态 ACTIVE map；`tick()` 由 WandscapeClient 驱动；`followBeam` 若存在 effectId 对应的光束则跟随其源点与朝向；`followCaster` 若法阵带 casterUuid（脚下法阵）则每 tick 跟随施法者实体位置（NPC 走位时法阵跟着移动）。`spawnElement`：localTime→采样 alpha(≤0.001 跳过)/scale/rotation；intervalTicks 脉冲门控；按 type 分发（GLYPH→spawnGlyph、POLYGON/STAR→spawnShape、RING/ARC→beads 或 continuous 带 ±0.2 抖动拖尾）。
...
- `NpcSpellPowerHandler` 用 SPELL_POWER 放大 NPC 施法伤害、再乘魔力强化独立乘区（`MagicSpellExecutors.magicEnhanceMultiplier`，每级 +20%）；`heal` 治疗量同样按施法者 SPELL_POWER × 魔力强化放大（`MagicSpellExecutors.castHeal`）。
