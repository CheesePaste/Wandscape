...
- `BuildingGhostRenderer`：放置虚影渲染门面，GHOST_ALPHA=0.40f；几何由 `BuildingGhostVboCache` 按 (config, rotationSteps) 预烘焙成 VBO（每帧单次 draw call），`drawGhostSkipped` 用退化三角形按世界采样跳过已建 cell。
