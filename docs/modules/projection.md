...
- `ProjectionRenderer`：AFTER_TRIPWIRE_BLOCKS；用 BuildingGhostRenderer 渲染半透明幽灵方块（几何由 `BuildingGhostVboCache` 按 (config, rotationSteps) 预烘焙成 VBO，一次性 tessellate、每帧单次 draw call），旋转后 boundary 先做整栋 AABB 视锥剔除，再画白线框（pinned 非重叠）/红框（重叠）。
