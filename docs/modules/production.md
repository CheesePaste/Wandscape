...
- `executeDecompose`：先校验 count×总价值 ≥ 10（否则拒绝，不扣物品），再扣物品（bank.consume），产出 = `mappings.getItemElementValue(itemId)`（decompose_yield→build_cost 回退，与商店同源）× 1/10 向下取整，**加到 colonyResources.addResource(元素名 ResourceId)**（非 ColonyItemBank 元素）。
