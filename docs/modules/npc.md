...
- **盔甲格**：`armorInventory`（SimpleContainer 4，顺序 头盔/胸甲/护腿/靴子），与 vanilla 装备槽分开存放 → 法袍外观不被覆盖（外形不渲染，仅数值生效）。`armorValueOf(stack)` 累加物品原版 `Attributes.ARMOR` 修饰符得单件护甲值；`syncArmorAttributes()` 把 4 槽护甲值以加法修饰符写回 ECS EquipmentComponent（HEAD/CHEST/LEGS/FEET），GUI 显示与伤害减免（vanilla ARMOR）都生效。**铁魔法装备的属性加成**经 `IronSpellsAttributes.modifiersFor` 一并映射进 ECS——`MAX_MANA`/`SPELL_POWER`/vanilla `MOVEMENT_SPEED` 三条（百分比映射 `MULTIPLY_BASE` 乘区），各学派法术强度/施法时移速等铁魔法特色属性不映射。onNpcJoinWorld（含重连/延迟注册）后调用。
