...
- 仇恨表 `setHatedAttacker/getHatedAttacker`；**友军名单** `isFriendlyForce`（同 colonyId 的 NPC + 所有玩家，纯逻辑 `core/types/FriendlyForce`）；反击目标判定 `isRetaliationTarget = !isFriendlyForce`（不要求 Enemy——北极熊/铁傀儡/狼等中立生物主动攻击也还手）；`canBeamHurt` 默认伤 Enemy 或当前仇恨目标；所有 NPC 伤害入口（`guard/NpcSpellPowerHandler`）统一取消友军伤害（铁魔法/召唤物也走此入口）。
