...
- **7 属性字段**：maxHp/moveSpeed/spellPower/workSpeed/spellSpeed/armorValue/maxMana，默认 `NpcAttributes.defaults()` = (30, 0.3, 1, 1, 1, 6, 200)。vanilla 基础属性：MAX_HEALTH 30、MOVEMENT_SPEED 0.3、ATTACK_DAMAGE 1.0、FOLLOW_RANGE 48。
...
- **装备/属性权威**：ECS `EquipmentComponent` 是运行时权威。`getEffectiveAttribute` 有 ECS 则取 eq.getAttribute，否则回退 NBT transit 字段；`applyEffectiveAttributes()` 仅变化时 setBaseValue MAX_HEALTH/MOVEMENT_SPEED/ARMOR（3 个 applied 脏值防重复）——ARMOR 只推天生+法杖加成（core ARMOR_VALUE 已不含槽内盔甲），槽内盔甲由原版结算叠加 transient，总护甲 = base + 槽内。
...
- **盔甲格**：盔甲直接存 vanilla 装备槽（HEAD/CHEST/LEGS/FEET，经 `setItemSlot` 写入）→ 原版每 tick 装备结算自动应用护甲值/韧性/击退/移速与附魔，其它模组也能读到槽位；巫师袍外观不被覆盖（渲染器无盔甲层，见 `WandscapeNpcRenderer`）。原版 `LivingEntity.hurtArmor` 对非玩家生物是空实现，耐久由 `hurtArmor` 覆盖按原版语义手动结算。**铁魔法装备的属性加成**经 `IronSpellsAttributes.modifiersFor` 桥进 ECS EquipmentComponent（HEAD/CHEST/LEGS/FEET）——`MAX_MANA`/`SPELL_POWER`/`COOLDOWN_REDUCTION`/`CAST_TIME_REDUCTION`（后两条折叠进 `SPELL_SPEED`，与"冷却/吟唱 ÷ SPELL_SPEED"同语义；百分比映射 `MULTIPLY_BASE` 乘区；vanilla `MOVEMENT_SPEED` 已由原版结算、不再映射），各学派法术强度/施法时移速/mana_regen/法术抗性等铁魔法特色属性不映射。onNpcJoinWorld（含重连/延迟注册）后调用。
- NBT 存 SkinVariant/HatColor/EcsEntityId/7 属性/魔力状态（currentMana/manaRegenAccum/spellLockTicks/magicCooldowns/manaSeeded）/回血/hasDefaultWand/colonyId/**PeaceMode/FollowMode/FollowerUuid**；盔甲由原版 `ArmorItems` 持久化，旧存档自定义 `armorInventory` tag 在 onAddedToLevel 迁移进 vanilla 槽。
- 仇恨表 `setHatedAttacker/getHatedAttacker`；**友军名单** `isFriendlyForce`（同 colonyId 的 NPC + 所有玩家 + 同殖民地 NPC 召唤的铁魔法随从 + 同殖民地游客，纯逻辑 `core/types/FriendlyForce`）；反击目标判定 `isRetaliationTarget = !isFriendlyForce`（不要求 Enemy——北极熊/铁傀儡/狼等中立生物主动攻击也还手）；`canBeamHurt` 前置友军判定（友军永不受伤）后默认伤 Enemy 或当前仇恨目标；所有 NPC 伤害入口（`guard/NpcSpellPowerHandler`）统一取消友军伤害（铁魔法/召唤物也走此入口）。
...
- `NpcDataPacket`（S→C）：信息屏，含 entityId/名字/血量/7 属性/wandStack/isDefaultWand/strategyPreset+魔法表+优先级/armorStacks(4)/skinVariant/hatColor/**peaceMode/followMode**；from() 从 ECS EquipmentComponent 读有效属性（护甲改读 vanilla ARMOR 有效值 = 天生 + 槽内盔甲）。
- `NpcEquipPacket`（C→S）：equip/unequip wand + equip/unequip armor（4 槽）。handleEquip 校验 WandItem、读 wand preset 的 attributes（均为 ADDITION），换物品并同步 ECS eq.unequip/equip；handleUnequip 拒绝卸默认 wand，回默认 wand。handleEquipArmor 用 `npc.getEquipmentSlotForItem` 判定盔甲槽（不信任客户端）、交换物品并 `syncIronArmorAttributes()`；handleUnequipArmor 回收背包。
