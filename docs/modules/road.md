...
殖民地道路网络：建筑间自动生成路网（MST）、玩家手动铺路/填平/销毁、样条编辑器、NPC/物品沿路寻路、物品运输动画。核心数据模型纯 Java，MC 实现在 `road/engine/`。
...
- `RoadNode(nodeId, GridPos, NodeType)`：NodeType BUILDING/INTERSECTION/ORPHAN/PLAYER。
...
- 支撑：CurveSample/PathPoint(3D 带 Y)/XZPoint(2D)/SplinePointCache/RoadBuildingData/RouteSegment(遗留)/SplineLeg(spline,uStart,uEnd,offRoad)。

## 算法（road/algorithm/）

- `MstCalculator`：Prim 算法求 XZ 点集最小生成树，distanceFn=曼哈顿，返回 MstEdge(fromIndex,toIndex,distance)。
- `PathGenerator`：L 形路径 lShape/lShape3D（平滑缓坡 + 共享走廊 Y 吸附 + 坡度>45° 回退 spiralPath 螺旋）、turnIndices3D。
- `RoadPlanner`：`computeMST(buildings, threshold, amplitude)`——建筑数 < threshold 返回空网络；建 BUILDING 节点，Prim+MST 生成 tier="dirt" 的 RoadEdge；`incrementalAdd` 连新建筑到最近可走点；`rebuild`→NetworkDiff(retained,deprecated,newEdges)；splitIntoSegments/filterNewPath。
...
- `NetworkDiff(retained, deprecated, newEdges)` record。

## MC 实现（road/engine/）

- `RoadApiImpl`：getNetwork/getEdges（从 overworld RoadSavedData）、requestFullRebuild→RoadEventListener.triggerRebuild、requestIncrementalUpdate(空 V1)、getBuildingThreshold、getBlobCache、removeEdge。
- `RoadBuilder`：buildTiles 实际铺路：每中心点圆形笔刷(width/2)、建筑包围盒避让、水面→oak_planks 于水面高度、pickFromPalette（Splitmix64 按 XZ 确定性取色）、净空挖掘（actualY+1..clearTop→air）、高架路下按 spacing 生成桥墩（碰到 occupied 块即停）。输出 JSON tiles `{pos:[x,y,z],block}`。
- `RoadSavedData`（`wandscape_roads`）：仅持久化边（spline 的 a/p/n/l、segmentTaskIds、placedBlocks、status、width），节点加载时重建；兼容旧 "path" 字段。
- `RoadTaskSource`：轮询式 TaskSource（20 tick，优先级10），drain 队列发布 `road:build_segment` 任务。
- `RoadEventListener`：订阅引擎 CustomEvent：`build_complete`→首次 MST（网络空且建筑数≥阈值）或增量 incrementalAdd；`road_segment_complete`→recordSegmentComplete，全完成置 COMPLETE；triggerRebuild→RoadPlanner.rebuild；enqueueEdge 拆段+建 tiles+入队。
...
- `RoadConfig`：读 Config TOML。数值：buildingThreshold=3、segmentMaxLength=16、defaultWidth=3、maxCutDepth=8、maxFillHeight=6、pillar 开/spacing 4/stone_bricks；装饰 enabled/lampSpacing 8/benchSpacing 24/lampPost oak_fence/lampLight lantern/benchBlock oak_stairs；surfacePalette 默认 `stone_bricks=50,andesite=25,stone=25`。**注意：getDecorationConfig 无调用方，装饰生成未接入 RoadBuilder。**
...
- > **注意**：`road_templates/`、`road_tiers.json`、`road_rules/` 当前**无代码读取**（RoadTemplate 由代码构建；RoadPresetLoader 只读 road_presets 类别，无对应文件）。JSON 结构见 [data/road.md](../data/road.md)。
