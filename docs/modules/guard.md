...
- `GuardCombat.engage`：有活跃光束→beam.retarget 主动切换；LOS 被挡→光束快淡出 + navigateToward；LOS 通→cancelNavigation + canCastSpell→castNpcAt，成功后 startSpellCooldown(CAST_MIN_INTERVAL=40)（实际 CD=基础/SPELL_SPEED）+ 杖尖 burstColored + GUARD_FIRE 音。
