...
- `GuardCombat.engage`：有活跃光束→beam.retarget 主动切换；LOS 被挡→光束快淡出 + navigateToward；LOS 通→cancelNavigation + `MagicCaster.castNpcAt`（CD/蓝/锁在 MagicCaster 内部门控：光束 50 蓝、基础 CD 40/SPELL_SPEED、施法互斥锁全程），成功后杖尖 burstColored + GUARD_FIRE 音。
