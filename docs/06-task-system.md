...
文档编号：NEW-06
版本：1.1
状态：全局任务池 + 调度器 + 私有池 + 生命周期 + 建筑队列 + NPC 死亡续传
依赖：01-shared-api
...
NPC 死亡时（`RemovalReason.KILLED`），持有的全局任务通过 `GlobalTaskPool.releaseTaskForReassign()` 处理：
- **保留进度**：`stepIndex` 写入任务记录，不丢失
- **丢弃私有池**：私有任务随 ECS 组件一同移除，不保留
- **重新入队**：任务状态设为 `PENDING_ASSIGN`，`assignedNpcId` 清空
- 下一个空闲 NPC 可在调度心跳中接取，从 `stepIndex` 继续执行
