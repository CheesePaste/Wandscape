...
- **短距离走位不传送（walkOnly）**：战斗走位/交战接近导航标记 `NavigationState.walkOnly`，`NavigationSystem.switchToRitualTeleport` 对 walkOnly 直接 `nav.reset()` 站定、不回退 self_teleport——否则落点仅 6~10 格远，寻路失败/卡住/超时就触发一次传送白烧 CD+30 蓝（用户实测「走位放极小距离传送」）。后撤落点 `findRetreatPos` 增加「NPC→落点 无墙」LOS 约束（走过去可达），源头减少寻路失败；长距离任务导航不置 walkOnly，仍可传送兜底。
