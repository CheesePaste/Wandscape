...
**决策**：
- **保真分级 = 导出源分级，而非内容黑名单**：`CreativeScannerBlockEntity.isSafeExport()`（默认 false，完整导出）→ `ScannerBlockEntity` 覆写为 true（纯建筑）。创造扫描器无配方、只在创造栏，生存玩家无法获得——「生存导出无 NBT」才不牺牲创造端保真，也不需要枚举任何模组容器的 NBT key，从根上避免「和别的模组一起加载防不住」的黑名单军备竞赛。
- **生存导出（isSafeExport=true）**：不写 `block_nbt`（方块实体 NBT 一律不要——容器、牌子文字、旗帜图案都打印为裸态）；装饰实体保留（画/框的摆放、朝向、旋转），但**剥掉物品展示框 NBT 里的 `Item`**（含 glow_item_frame），封死「框里放珍贵物品→打印复制」这一条平行口子。
- **创造导出**：行为不变，完整保真，供创作者存档/编辑/分享。
- **不加建造端兜底**（用户确认）：`setBlockEntityData` 仍按 JSON 原样应用 NBT——威胁模型是「防无文件权限的生存刷物品」（已由导出分层封死）；文件级注入（手改 world datapack 里的 JSON / 第三方后台塞包）本质是"信任文件"，这类玩家本就可编辑 datapack 任意取得方块，堵不住也没必要堵。

**影响**：`CreativeScannerBlockEntity` + `isSafeExport()`、`ScannerBlockEntity` 覆写、`ScannerExportPacket` 生存分支（跳过 block_nbt + 剥展示框 `Item`）。`docs/data/buildings.md` 的 `block_nbt` 注释同步标注来源限制。无测试（改动为 MC 耦合的导出路径分支，无纯净逻辑；判定本身是平凡 getter，按「不值得测」原则不加）。
