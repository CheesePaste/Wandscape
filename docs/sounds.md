...
| 挂点 | 文件:方法 | 触发 | 建议音效 |
|------|-----------|------|----------|
| 方块放置/移除 | `engine/boundary/WandscapeBlockOps.java` `setBlock()`:59-68 | 同步/异步建造/拆除都汇此 | 放置：方块自身原版放置音 `getPlaceSound()`；移除（目标为空气）：被拆方块原版破坏音 `getBreakSound()`。两者都走 `SoundService.playAtThrottled` 节流 10 tick，`BLOCKS` |
| NPC 施法放置 | `engine/boundary/AsyncTransformExecutor.java` `execute()` `thenRun`:99-115（`doWorkAnimation` :110 处） | NPC 放置每块方块 | 自定义施法音，`NEUTRAL`，与方块放置/拆除音同频节流 10 tick（`NPC_CAST_THROTTLE_TICKS`）防整栋楼连响刷屏 |
...
### 4.4 环境音（不依赖事件，服务端近距门控 + 客户端循环）

殖民地有白昼/夜晚两套**循环环境音**，不依赖任何事件：

| 相位 | 时间窗口 | 音效 | 音量 |
|------|---------|------|------|
| 白天 | dayTime ∈ [1000, 18000)（游客在城；对齐 `tourist.spawnWindowStart`=1000 / `tourist.departureWindowStart`=18000） | `colony_ambient_day`（人群低语） | 0.6 |
| 夜晚 | 其余时间（游客离城） | `colony_ambient_night`（森林，低音量） | 0.4 |

**门控机制（服务端→客户端）**：
- `building/internal/ColonyAmbientTracker.java` 每 20 tick 评估：玩家位于**任一建筑包围盒向外延伸 20 格**的范围内即视为"在城镇内"。进入/离开或相位切换时发 `ColonyAmbientPacket(playing, day)`；在城玩家每 120 tick 心跳重发防丢包。
- `engine/sound/ColonyAmbientSystem.java`（客户端）收包驱动循环音启停/切相位；`tick()` 仅兜底：世界卸载或切维度时停止。

**音频与播放**：
- 音源是长循环 `.ogg`（`ambient/colony_day.ogg`、`ambient/colony_night.ogg`），sounds.json 里 `"stream": true` 流式播放；两文件均经 ffmpeg `loudnorm` 归一化（原录音过轻）。
- 播放实例为 `AbstractTickableSoundInstance`（`looping` + `relative` + `SoundSource.AMBIENT` 通道），相位切换停旧启新并淡入。
- 门控半径改 `ColonyAmbientTracker.TOWN_RADIUS`；相位分界改 `DAY_START_TICK`/`NIGHT_START_TICK`（对齐 Config 游客窗口）。
