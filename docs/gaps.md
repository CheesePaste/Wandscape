...
1. **「声望(reputation)」机制虚构**——指南把"声望"当真实机制讲，但代码无 reputation 概念：
   ...
   真实机制：游客生成数 = 均匀整数区间 `[base(4)+(colonyLevel−1)×levelSpawnBonus(1), +spawnRangeWidth(3)]`（1 级 4~7、2 级 5~8），等级分布固定 40%(colonyLevel−1) / 40%(==) / 20%(+1)，与"声望"无关。修正方向：改为"殖民地等级影响游客生成数量与等级"。
