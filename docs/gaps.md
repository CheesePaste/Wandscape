...
| 9 | `BuildingDebugController` "G key" 开关 | G 键的 overview 切换已移除；BuildingDebug 激活由 V 面板驱动，且**显示范围收窄到俯瞰(OVERVIEW)模式**（2026-08-28，`isInspectContext`）——建造/道路/统计/任务子模式不弹建筑信息顶栏 |
