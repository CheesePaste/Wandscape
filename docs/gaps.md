...
强加载方案覆盖"必须放置方块"的行为（建造/生产/采集/拆除）+ 游客个人级 sim（1.3.0，逐游客按区块判定）。以下仍只在区块加载时运行，属已知取舍：
- 守卫/袭击：物理任务，依赖区块加载。
- NPC 跨重启持久化：库存/私有任务队列不落盘（既有缺口，另开阶段）。

### 游客 sim 已知缺口（2026-08-05，前两点已修复）
- ~~影子 spawn 实体时外观/皮肤不还原~~ → 已修（1.3.1 加 setSkinVariant/setAppearance，importToEntity 恢复）。
- ~~过夜统计 countOvernightStayers 只数加载实体~~ → 已修（1.3.1 计入影子住店）。
