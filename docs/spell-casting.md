...
- **无死亡记录前置校验**：`AltarCastHandler.onCastRequest` 点选 revive 时若 `ColonyDeathRegistry` 为空直接提示"没有可复活的死亡记录"、不发布任务；`AltarCastExecutor` 幂等复核兜底——发布后记录被其他祭坛复活消耗/过期时跳过施法（不扣蓝、不放法阵）。
