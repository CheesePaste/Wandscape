...
| [networking-survey.md](networking-survey.md) | **网络包系统考察报告**：86 包/10233 行的现状硬数据与六个病灶（410 行注册墙、83/86 手写 codec、16 处 raw NBT、31 处 handler 静态注入、可合并包族、缺通用设施）、10 个参考模组横评（行业默认交还原版 openMenu 与 BE 同步）、改进选项分「该动/挑着做/别做」三档；**§六 面向移植的收口方案**——155 文件的平台接触面 vs 仅 ~12 个 API 符号、`Net` 发送门面 + `PayloadRegistry`、机械成本交一次性脚本而非常驻抽象层、三条明确别做 | 想动网络层、觉得「包太多太啰嗦」、或评估网络层对移植（降级 1.20.1 / Fabric / 升 26.1）的拖累时 |
...
| [guidebook-screenshots.md](guidebook-screenshots.md) | **手册配图的拍摄清单**：66 个条目的逐张截图位（截哪个界面、从哪进、画面上必须出现什么）+ 拍摄与出图规格（100×100 取景、256 画布、字高约束）+ 分页器吞图片页的实测缺陷与修复方向 | 给手册补真实截图、或发现加了 `![]()` 后图不出现时 |
