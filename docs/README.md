...
| [touhou-little-maid-compat.md](plan/touhou-little-maid-compat.md) | **车万女仆兼容可行性与分阶段方案**：两侧扩展面与硬耦合点、阶段一（盟友 + 女仆工作模式）的分步落地与状态、阶段二（法师小屋/法杖施法/策略槽）改造清单、四项前置验证实测、任务类型准入（`caster_only`）。含顺带开放的 `ColonyWorkerApi`（任意 `Mob` 登记为殖民地工人，见 api-ledger §23） | 做第三方实体兼容、抽 `ColonyWorker` 缝、让其它模组的生物当工人、或评估女仆/附属模组接入前 |
