...
### 0.2 尺寸现实：整屏截图进书里读不出字

书页文字区只有 **116×156 逻辑像素**、字高 8px。而 `PageImage` 把源图**只取左上
200×200、再拉伸成 100×100**（`blit` 的 UV 分母硬编码 256，故采样 UV 0..0.78125）。所以：

- 截图必须先等比缩放进 200×200 **再摆到 256×256 画布左上角**——否则右边和下边各约 22% 不显示，且被拉成方形；
- 即便如此，591×459 ~ 1093×649 的整屏截图缩完**字高只剩 1.5–2.7px**（`prepare_guidebook_images.py` 会逐张报）。

**当前这批是「示意」而非「可读」**：读者看得出「这个界面长这样、有这么几处」，
但读不出面板上的任何一项数值。要让文字可读，得改成裁局部特写（源图裁约 230×310 的一块，
2× 下采样后字高仍是 8px）——代价是丢掉整体版面。**取舍待定，现状是整图。**
...
100×100 逻辑像素**比一个完整面板小得多**——塞不进「管理面板全貌」这种东西，塞进去也读不清。**每张图只讲一个控件**：一个按钮、一行槽位、一列标签，四周留一圈上下文即可。凡是想拍「整个界面」的，改成拍「那个界面里读者要找的那一处」。

### 1.3 落位与引用

```
guidebook_screenshots/<名字>.png                                     原始截图（仓库根，不进 jar）
src/main/resources/assets/wandscape/textures/guidebook/<名字>.png    加工成品（256x256，打进 jar）
```

**原始截图放仓库根、且已 `.gitignore`**（`.gitignore` 的 `guidebook_screenshots/` 那条），
跟 `_refs/`、`balance/` 一个待遇：它是**源素材**不是运行时资产。
放 `src/main/resources/` 里会白打进 jar（1.8MB），而游戏真正要读的是
`prepare_guidebook_images.py` 产出的 564KB 成品。**截图本身不进仓库**——
换人接手要重新拍，脚本和清单都在这儿，照着走即可。
...
| 56 | `item_scepter_tip` | P2 | 悬停权杖 | 提示里当前模式的显示；万能权杖那张加 shift 右键的说明 |
