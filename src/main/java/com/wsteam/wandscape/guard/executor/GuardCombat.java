...
public final class GuardCombat {
     ...
     *   <li>LOS 可见 → 停止移动；无光束且距上次施法足够久 → {@link MagicCaster#castNpcAt} 施法。</li>
     * </ol>
     *
     * @param lastCastTick npcId → 上次成功施法的 gameTime（施法节流，跨轮共享）
     */
    public static void engage(ServerLevel level, WandscapeNpc npc, LivingEntity target,
        ...
        if (beam == null && level.getGameTime() - lastCastTick.getOrDefault(npcId, 0L) >= CAST_MIN_INTERVAL) {
