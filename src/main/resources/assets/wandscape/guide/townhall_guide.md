...
| :--- | :--- | :--- | :--- |
| **`Colony Name EditBox`** (`nameBox`) | String (`Max 32 chars`) | `My Colony` | 殖民地名称文本输入框。修改后按下 Enter 或失去焦点，向服务端发送 `ColonyNameUpdatePacket` 更改全局殖民地名称。 |
| **`Colony Level`** (`stat_level`) | Display Badge (`1 ~ 5`) | `1` | 展现当前殖民地等级。等级提升可增加全局人口上限与解锁高级建筑蓝图。 |
| **`Experience Bar`** (`stat_exp`) | Progress Bar (`0 ~ 100%`) | `0/1000` | 展现殖民地建设与经营经验进度 (`experience / expToNext`)。经验满后触发市政厅升级。 |
