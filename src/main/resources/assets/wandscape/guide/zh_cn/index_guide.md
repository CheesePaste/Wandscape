# 📖 Wandscape 指南

欢迎来到 Wandscape！如果你正在读这本书，说明你多半已经站在一块空地上，盘算着把它变成一座魔法小镇——恭喜，这确实是个好主意。

Wandscape 是个模拟经营模组：你放下建筑，**法师 NPC 替你干活**（建造、采集、合成、守卫），**游客沿路入城消费**。你要做的，是决定小镇往哪走。这本书按 **H** 键随时打开。

> 第一次玩？先读[新手入门：从空城到游客小镇](guide:getting_started_guide)。那篇把整个游戏循环讲透了，读完你就知道该干嘛。

## 🏗️ 建造与城镇

- [全局面板与鸟瞰视角](guide:overview_guide)——按 V 打开，一切从这里开始
- [建筑扫描器](guide:scanner_guide)——把建好的房子变成蓝图，让 NPC 照着重建

## 🛣️ 道路系统

游客只走铺好的路。**路是经济的命脉**，这句不是比喻。

- [道路系统总览](guide:road_guide)
- [替换模式（地表铺路）](guide:road_replace_guide)
- [填充与清理（Fill / Destroy）](guide:road_fill_guide)
- [样条线编辑器（弯道与立交）](guide:road_spline_guide)

## 🏘️ 建筑与设施

- [市政厅 Town Hall](guide:townhall_guide)
- [仓库 Warehouse](guide:warehouse_guide)
- [合成台 Crafting Station（造法杖）](guide:crafting_guide)
- [工作站 Workstation（分解与合成）](guide:workstation_guide)
- [元素节点 Node](guide:node_guide)
- [祭坛 Altar（复活）](guide:altar_guide)
- [酒馆 Tavern（招募法师）](guide:tavern_guide)
- [商店 Shop](guide:shop_guide)（机制重做中）
- [旅馆 Hotel](guide:hotel_guide)（机制重做中）

## 👥 NPC 与游客

- [法师 NPC 指南](guide:npc_guide)
- [施法策略指南](guide:strategy_guide)
- [游客系统指南](guide:tourist_guide)

## ⚠️ 经营与维护

- [建筑故障与维护](guide:anomaly_guide)

## 🛠️ 创作者工具

- [魔法阵 Web 编辑器](guide:magic_circle_editor_guide)
