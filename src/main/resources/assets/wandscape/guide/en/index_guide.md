# 📖 Wandscape Guide

Welcome to Wandscape! If you are reading this book, you are most likely standing on an empty plot of land, wondering how to turn it into a magic town — congratulations, that really is a good idea.

Wandscape is a management sim mod: you place buildings, and **wizard NPCs do the work for you** (building, gathering, crafting, guarding), while **tourists walk along the roads to spend in town**. Your job is to decide where the town goes. This book can be opened at any time with the **H** key.

> Playing for the first time? Read [Getting Started: From Empty Land to a Tourist Town](guide:getting_started_guide) first. That page explains the whole game loop — after reading it you will know what to do.

## 🏗️ Building & Town

- [Panel & Overview](guide:overview_guide) — press V, everything starts here
- [Building Scanner](guide:scanner_guide) — survival players turn houses they built into blueprints so NPCs can rebuild them
- [Creative Building Scanner](guide:creative_scanner_guide) — a creator tool: interact spots, business parameters, decoration export

## 🛣️ Road System

Tourists only walk on paved roads. **Roads are the lifeline of the economy** — that is not a metaphor.

- [Road System Overview](guide:road_guide)
- [Replace Mode (Surface Paving)](guide:road_replace_guide)
- [Fill & Destroy](guide:road_fill_guide)
- [Spline Editor (Curves & Interchanges)](guide:road_spline_guide)

## 🏘️ Buildings & Facilities

- [Town Hall](guide:townhall_guide)
- [Warehouse](guide:warehouse_guide)
- [Crafting Station (Wands)](guide:crafting_guide)
- [Workstation (Decompose & Synthesize)](guide:workstation_guide)
- [Element Node](guide:node_guide)
- [Altar (Revive)](guide:altar_guide)
- [Tavern (Recruiting Wizards)](guide:tavern_guide)
- [Shop (Selling to Tourists)](guide:shop_guide)
- [Hotel & Service Buildings (Overnight / Element Production)](guide:hotel_guide)
- Relax buildings (bathhouses etc., restore energy) and ATMs (refill travel money) — see the [Tourist System Guide](guide:tourist_guide)

## 👥 NPCs & Tourists

- [Wizard NPC Guide](guide:npc_guide)
- [Cast Strategy Guide](guide:strategy_guide)
- [Tourist System Guide](guide:tourist_guide) — the three need bars, profiles, and four types of tourist buildings

## ⚠️ Operations & Maintenance

- [Building Failures & Maintenance](guide:anomaly_guide)

## 🛠️ Creator Tools

- [Magic Circle Web Editor](guide:magic_circle_editor_guide)

## 📝 About

- [Creators & Special Thanks](guide:creators_guide)
