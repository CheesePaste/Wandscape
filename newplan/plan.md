...
| 7 | 归属判据 | **归属看语义，不看依赖方向**：类/包归哪问"它是什么、服务于谁"，不问"谁依赖它"；跨域直接调用 = 正常、= 默认，**永远不是把类挪出归属域的理由** | 2026-08-31 迁移定版实证：`raid` 本属 colony（殖民地袭击）却被"依赖 guard"带偏；`scepter` 本属 items（功能性物品含系统层）却被"npc/guard 反向依赖 items"挡下。两处都是旧 `shared` 桥层"为规避直接引用而搭桥"同病复发。**禁动机才能根治**——CLAUDE.md「代码组织」已加同文条款 |
