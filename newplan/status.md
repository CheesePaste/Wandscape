...
- **Tier 4 shared/ 桥层消解完成（2026-09-01）**：依据 `tier4-dissolve-rules.md` 将 `shared/` 目录下 139 个类全部消解至 `foundation/`、`content/<domain>` 与 `api/`，彻底清空并移除了 `shared/` 顶层目录。全仓 grep `com.wsteam.wandscape.shared` 零命中，`./gradlew compileJava` 100% 绿：
  - `shared/log/`、`shared/util/`、`shared/registry/`（除 WandscapeApis）、`shared/ui/`（markdown/component/theme/animation/skin/guidance/vanilla/util）及 `shared/client/bubble/`、通用包 → `foundation/`（`log`、`util`、`registry`、`ui`、`networking`）
  - `shared/data/`、`shared/event/`、`shared/entity/`、`shared/client/render/`、专属 UI 面板（BuildingSelection/TaskManagement）及各域网络包 → 分流至 11 个 `content/` 功能域（`building`、`colony`、`tourist`、`npc`、`task`、`element`、`magic`、`items`、`road`、`warehouse`）
  - `shared/api/` 及 `WandscapeApis` → 迁入 `api/` 顶层契约包
...
- **暂缓清单**：command（归域+debug 大清理）、顶层 client（TransportItemEntityRenderer 随 transport）、mixin（各归各域，依赖域内结构定后）、gametest（ElementAuditRunner 归 element）——与 core/engine 桥层拆解分开处理。
