...
怪物进入建筑 AABB 水平 +10 区（Y 不扩展）
  ...
  → 调度器派给空闲 NPC → GuardAttackExecutor 持续循环
  → 区域内找最近 Enemy → LOS 通过 → castNpcAt 施法 → 等光束结束 → 重选
...
| `executor/GuardAttackExecutor.java` | OpExecutor<AttackMonsterOp>：持续异步循环（施法→等光束→重选→再施法），+15 区无怪才 complete |
...
- 守卫任务 `AttackMonsterOp.target() = null` → 无 stance/导航，NPC 不走路。
