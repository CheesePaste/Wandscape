...
- **画像**：40% 均衡 `{1,1,1}`、20% 舒适 `{1.4,0.8,0.8}`、20% 魔法 `{0.8,1.4,0.8}`、20% 奇观 `{0.8,0.8,1.4}`。三条 need = `totalNeed × 权重占比`，`totalNeed = TOURIST_NEED_BASE + (level-1)×TOURIST_NEED_PER_LEVEL` —— **等级越高总需求越高、越难满足**（自然难度曲线）。
...
- 夜晚 + 未满条 → 只选旅店（`service.maxOccupancy>0` 且有空位）。
