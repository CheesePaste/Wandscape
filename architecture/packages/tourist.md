...
- **停留**：`departureDeadline = arrivalTime + rand(2~4)×24000`；`nightsStayed` 住店晚数。**`visitedBuildings` 停留期不重置**（防挂机，一栋建筑整个停留只逛一次；**豁免例外**——ATM `atmReusable`（缺钱）可分批取现、relax `relaxReusable`（精力低）可重复歇脚回精力，分别靠取现冷却/精力阈值控节奏，visited 本身仍累计）。
...
游客生成依赖道路系统元数据：RoadSavedData 边界路面位置生成。移动不再走路网路由——通勤（去建筑/POI）用 vanilla A* 直寻；路面只作为**方块条件**（`custom_roads` 标签）影响闲逛目标选取与移动速度。道路布局影响游客流量（生成点）与闲逛体验。
...
- **离路减速**：发出 moveTo 时脚下不是路面方块 → 速度 ×0.8，铺路有实际移动价值。
- 道路方块列表数据驱动（`data/wandscape/tags/block/custom_roads.json`，`replace:false` 可被数据包合并扩展）；物品运输直线采样判路与该标签共用。
