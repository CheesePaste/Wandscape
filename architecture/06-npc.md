...
| 文件 | 作用 |
|------|------|
| `entity/WandscapeNpc.java` | **NPC 实体**：继承 `PathfinderMob`。持有 ecsEntityId / colonyId / mana 属性 / 27 格背包。AI：FloatGoal(防溺水) + RandomStrollGoal(0.6 速度，受 `setAiWanderingEnabled()` 控制)。`FOLLOW_RANGE=32` 用于寻路搜索范围。生命周期 `onAddedToLevel`→ECS 注册，`onRemovedFromLevel`(KILLED)→`releaseTaskForReassign` 保留 stepIndex 重新入队，再 ECS 移除。NBT 持久化从 ECS ManaPool 读。**施法状态** `DATA_CASTING`（synced boolean）驱动客户端动画+粒子。**施法时面向目标**：每 tick 从 ECS `TaskExecutor.currentOpTarget` 读取操作目标 → `faceTarget()` 计算 yaw/pitch。**施法时锁定移动**（`getNavigation().stop()` + `setDeltaMovement(Vec3.ZERO)`）。**Debug 模式** `debugCasting`：右键切换，跳过 ECS 轮询强制施法，`DATA_DEBUG_TARGET` 同步目标坐标 |
