...
| 文件 | 作用 |
|------|------|
| `WandscapeEngine.java` | **单例**持有 World 实例 + AsyncTransformExecutor + BlueprintConfigLoader + WandscapeMovementOps + TaskPoolSavedData。`ServerStarting` 时 `EngineBootstrap.bootstrap()` 注入 + 加载持久化任务，`ServerStopped` 时 `reset()` 清除（支持退出世界后重新进入） |
| `TaskPoolSavedData.java` | **任务跨会话持久化**（SavedData）。`getOrCreate(level, pool)` → `save()` 序列化所有 active 任务（blueprintId + taskParams + stepIndex + state）→ NBT；`load()` 重新编译蓝图并恢复进度。`pool.onChanged` 回调标记脏数据 |
...
| `boundary/WandscapeRitualOps.java` | RitualOps MC 实现：**V2.5 异步引导**。PendingRitual 队列 + `tickAll()` 倒计时 → `thenRun` 执行。`self_teleport` 600 ticks 引导后传送。`channelTicks()` 硬编码与 `RitualOp.channelTicks()` 一致 |
...
Wandscape.onServerTick (Post):
  ...
  ② blockInteractExec.tickAll()    — BlockInteract 引导倒计时
  ③ ritualOps.tickAll()            — RitualOp 引导倒计时
