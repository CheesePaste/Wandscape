...
> 2. **行程「满意 ±X」拆成三条各自增量**：`VisitMemory` 增补 Comfort/Magic/Wonder 三 delta（契约补充，见 §5），面板行程逐维显示。
...
4. `TouristDataPacket` + `TouristScreen`：三条 bar + 画像标签 + 活动 + 已住晚数/停留天数 + 钱包/旅费；行程逐维增量；删满意度单条。
...
| `tourist/network/TouristDataPacket.java` | 扩展/替换字段（三条 sat/need、活动、停留、travelFund；删 satisfaction/currentState/target*）；`VisitEntry` 改三维 delta |
| `tourist/client/TouristScreen.java` | 三条 bar + 画像标签 + 活动 + 停留 + 钱包/旅费；行程逐维增量；删满意度单条 |
| `shared/data/VisitMemory.java` | **契约补充**：增 `comfortDelta/magicDelta/wonderDelta`（保留旧 satisfaction 字段到 Block 3/4 过渡） |

## 契约补充（跨块协调，须回写 Block 3 消费方）

- **`VisitMemory` 增三 delta**（§5）：Block 2 改 record + Builder + 实体/影子 NBT；**Block 3 构造 `VisitMemory` 时填三 delta**（`TouristSimulation`/`TouristSimSystem`/`TouristMoveGoal` 的 `VisitMemory.builder()` 调用点）。
...
- **`registerDeparture` 仍 3 参**：`TouristApi.registerDeparture(UUID, UUID, int satisfaction)` 签名由 Block 4 收口；Block 2 沿用现有 3 参调用，第三参传聚合值 `barRatioPct = floor(min(sat_d/need_d) × 100)`。
...
```
> 依赖：`getSatisfaction()` stub 返回聚合值 → `onTouristKilled`（:440）与 `registerDeparture` 调用无需改签名；`TouristSimulation`/`TouristMoveGoal`/`TouristSimSystem`（Block 3 文件，未合并）读 `getSatisfaction()` 编译不破。
...
- `registerDeparture(t.getUUID(), colonyId, barRatioPct)`（:569，第三参改传 `floor(minRatio×100)`；签名不变，Block 4 收口）。
...
- `VisitEntry`（:49-61）：`(buildingTypeId, buildingName, whatHappened, comfortDelta, magicDelta, wonderDelta, energyDelta)` —— `satDelta` → 三维 delta。
...
- 布局（自上而下）：
  ...
  6. **冷却**（保留，:110-113）。
  7. **行程**（:115-147）：每行 `建筑: 事件 (舒适±c 魔法±m 奇观±w · 精力±e)`；`formatDelta` 复用，删「满意」字样。
- `apply`（:49-58）：删 `satisfaction`，新增字段赋值。

### 5. VisitMemory（契约补充，`shared/data/VisitMemory.java`）
...
public record VisitMemory(
        ...
        String category,
        long gameTime,
        int comfortDelta, int magicDelta, int wonderDelta,   // 新增：逐维填充增量
        int satisfactionBefore, int satisfactionDelta,       // 过渡期保留（Block 3 文件仍构造/读取；Block 4/5 删）
        int energyDelta,
        String whatHappened,
        Emotion emotion
)
```
- `Builder` 增 `comfortDelta/magicDelta/wonderDelta(int)` 三方法；`build()` 带新字段。**旧 `satisfactionBefore/satisfactionDelta` builder 方法保留**，保证 Block 3 的 `VisitMemory.builder()` 调用点（`TouristSimulation`/`TouristSimSystem`/`TouristMoveGoal`）编译不破。
- `emotion` 维持 `Emotion.fromDelta(satisfactionDelta)`（过渡期；Block 3 改为三维后随删）。
- 消费方（Block 2）：
  - `TouristEntity` load `new VisitMemory(...)`（:632-642）与 `TouristShadow` load（:385-394）补新参；save 增 `comfortDelta/magicDelta/wonderDelta` key（:520-534/:307-320）。
  - `TouristDataPacket.from` 从 `visit.comfortDelta()...` 填 `VisitEntry`。
- **Block 3 协调项**：构造 VisitMemory 时填三维 delta（如 `fillBars` 返回三维增益）。
...
3. `registerDeparture` 保持 3 参调用（第三参聚合值）；`NarrativeGenerator.generateDeparture` 保持现有签名。
...
5. TouristScreen 显示三条 bar + 画像标签 + 活动 + 已住 N 晚/共 X 天 + 钱包/旅费；行程逐维增量；无单一满意度。
