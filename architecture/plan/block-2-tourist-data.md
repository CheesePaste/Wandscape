...
> 依赖 Block 0 契约（TouristStateHost default 方法 + Activity 枚举 + 四类模式预设）。**只做数据面**：存储字段 + NBT + 生成/离开 + Screen。**不碰** TouristSimulation/TouristMoveGoal/TouristSimSystem/HotelStayHandler（Block 3）。本块与 Block 1/3/4 可并行。

## 目标

1. TouristEntity/TouristShadow 实现 TouristStateHost 新 default 方法（真实字段 + NBT）。
2. **删除 `satisfaction`、`typePreferences` 字段与 NBT key**。
3. TouristSpawnSystem：生成时 roll 画像 + 设停留截止 + **设 travelFund（ATM 取现池）**；离开判定改为「满条/截止/无恢复/无床位/idle」。
4. TouristDataPacket + TouristScreen：显示三条 bar + 画像 + 活动 + 停留天数 + 钱包/旅费余额；删满意度单条。

## 负责文件

| 文件 | 动作 |
|---|---|
| `tourist/entity/TouristEntity.java` | 加三条/画像/活动/停留/**travelFund** 字段 + NBT；删 satisfaction/typePreferences |
| `tourist/internal/TouristShadow.java` | 同上（影子副本） |
| `tourist/internal/TouristSpawnSystem.java` | 生成 roll 画像+设截止+设 travelFund；离开判定重写（D6） |
| `tourist/network/TouristDataPacket.java` | 扩展字段（三条/画像/活动/停留/travelFund） |
| `tourist/client/TouristScreen.java` | 三条 bar + 画像标签 + 活动 + 停留天数 + 钱包/旅费余额 |
...
private int travelFund;                             // 总旅费（ATM 取现池）；生成时 = startingWallet × TOURIST_ATM_TRAVEL_FUND_MULTIPLIER
```
...
**NBT save/load 新 key**：`comfortSat/magicSat/wonderSat/comfortNeed/magicNeed/wonderNeed/activity/activityTicks/occupiedSpot/nightsStayed/departureDeadline/travelFund`。删除旧 `satisfaction/typePreferences` key。
...
- 删 `satisfaction`、`typePreferences` + NBT key。
- 加三条/画像/活动/停留/**travelFund** 字段 + NBT（影子存绝对 simTick 基；departureDeadline 存绝对值即可）。
...
// 4) 总旅费 = 随身现金的倍数（ATM 取现池，防无限取现）
int starting = startingWallet(ps.level);
tourist.setWallet(starting);
tourist.setInitialWallet(starting);
tourist.setTravelFund((int) Math.round(starting * Config.TOURIST_ATM_TRAVEL_FUND_MULTIPLIER.get()));
```
...
```
- **精力 0 且无 relax 恢复建筑 → 不在此离场**：改为闲逛（Block 3 行为），直到视野出现 relax 建筑 / 夜晚入旅店 / 截止。
...
- `TouristDataPacket.apply`（:49-58）字段扩为：comfort/magic/wonder sat 与 need、currentActivity、nightsStayed、停留天数、wallet、travelFund；删 satisfaction。
- `TouristScreen`：删满意度单条（:94-99）→ 加三条 `drawStatBar`（Comfort/Magic/Wonder，显示 fill/need）；加画像标签（如「偏爱魔法」= need 最高维）；加活动状态文本（Activity 名称）；加「已住 N 晚 / 共 X 天」；钱包显示旁加「旅费 travelFund」余额。
...
3. TouristScreen 显示三条 bar + 画像 + 活动 + 停留天数 + 钱包/旅费余额。
4. 生成时画像随机且**按等级缩放总需求**（高等级总需求更高）；截止时间在 2-4 天内；travelFund = startingWallet × multiplier。
