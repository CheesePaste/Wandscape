...
- **Wand system** — one wand item, NBT-defined behavior tags & tiers, unbreakable, dynamically tinted, JSON recipe driven
...
- **Task system** — 8 atomic operations + a blueprint DSL; a global task pool scores and dispatches by distance/efficiency/stats, auto-triggering gather or craft when resources run low
...
- **Casting** — trigger via wand right-click or debug command; the circle renders perpendicular to the wand, and a tintable beacon beam fires at the crosshair target after the animation
- **Example** — ships with the `arcane_hexagram` large summoning-hexagram spec
- **Role** — visual layer of the combat (guard) system, reusable by wand, command and rituals; beam damage arrives in a later guard-system phase
