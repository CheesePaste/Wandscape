...
当前开发方向由 git 分支决定（`git branch --show-current`）：

| 分支 | 方向 |
|------|------|
| `main` | 修 Bug（主线稳定，不做新功能/大重构） |
| `feat/1.9-remove-imgui` | 移除 ImGui 依赖，道路工坊迁移原生覆盖层 |
| `feat/1.9-jei` | JEI 兼容（法杖标签页展示元素合成/分解配方） |
| `feat/1.9-cleanup` | 代码清理与包结构修整 |
| 其他分支 | 按分支名/内容自行判断 |

**规则**：每次任务开始前先确定当前分支方向。若用户请求**明显**属于其他分支方向（如 main 上做新功能/大重构，或功能分支上修 bug），**先提示用户切到对应分支**再动手；属于本分支方向则直接继续。
