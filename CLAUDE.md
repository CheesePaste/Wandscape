...
6. **引擎是请求层，适配层是实现**：`core/` 包只定义接口和数据模型。所有 MC 层面的实现逻辑（定时器、寻路、粒子、传送、GUI）必须在 `engine/` 或各模块 `internal/` 中实现。core 禁止引入 `java.util.concurrent` 之外的并发/调度工具，禁止持有任何运行时状态（计数器、缓存、pending 列表）——状态由适配层管理，core 只提供 `startAsyncOp()` 等无状态的请求-响应原语。
