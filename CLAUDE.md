...
- 重构动机与问题清单：`newplan/why`
- 进度跟踪：`newplan/status.md`（每个阶段做完立即更新）
...
| `newplan/` | 重构动机（`why`）与进度（`status.md`） | 重构期间每次必读 |
...
5. **参考模组查证**：动**结构/包组织/API 面/UI 形态/数据组织**类改动前，先到 `_refs/`（MineColonies/Create/Botania 浅克隆，自 2026-08 起挂在本仓库、已 gitignore）找对应做法作参照再动手；简单机械改动与纯 bug 修复不必。速查：殖民地/NPC/任务/建筑纵向切片看 MineColonies；顶层包形态/注册门面/API 门禁看 Create；代码量控制/API 稳定/注册工厂/Screen 精简看 Botania。三家解剖要点汇总在 `newplan/plan.md`【参考解剖判据】。**参考的是判据不是代码，禁止整段照搬**（版本/授权/结构都不同）。
...
6. 重构中发现的新问题，追加进 `newplan/status.md`；每次一个阶段做完，更新 status.md 的进度。